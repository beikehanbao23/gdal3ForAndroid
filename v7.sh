...
CC="ccache $NDK_TOOLCHAIN/bin/armv7a-linux-androideabi24-clang" ./configure \
  --prefix=/tmp/v7 --host=armv7a-linux-androideabi24
make -j3
make install
cd ..
...
cmake .. \
  ...
  -DCMAKE_INSTALL_PREFIX=/tmp/v7 \
  -DCMAKE_SYSTEM_NAME=Android \
  -DCMAKE_ANDROID_NDK=$ANDROID_NDK \
  -DCMAKE_ANDROID_ARCH_ABI=armeabi-v7a \
  -DCMAKE_SYSTEM_VERSION=24 \
  "-DCMAKE_PREFIX_PATH=/tmp/v7;$NDK_TOOLCHAIN/sysroot/usr/" \
...
PKG_CONFIG_LIBDIR=/tmp/v7/lib/pkgconfig cmake .. \
 -DUSE_CCACHE=ON \
 -DCMAKE_INSTALL_PREFIX=/tmp/v7 \
 -DCMAKE_SYSTEM_NAME=Android \
 -DCMAKE_ANDROID_NDK=$ANDROID_NDK \
